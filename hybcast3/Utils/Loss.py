...
class LevelVariabilityLoss(nn.Module):
  ...
  levels: tensor with shape (batch, n_time)
    levels obtained from exponential smoothing component of ESTransformer
...
class SmylLoss(nn.Module):
  ...
  windows_y_hat: tensor of predicted values,
                  shape (n_windows, batch_size, window_size).
  levels: levels obtained from exponential smoothing component of ESTransformer.
  ...
  def forward(self, windows_y, windows_y_hat, levels):
    # print('Debugging_BU')
    # print('Windows_y shape: ',windows_y.shape)
    # print('Windows_y_hat shape: ', windows_y_hat.shape)
