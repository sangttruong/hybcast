...
# Transformer
from Transformer.transformer import _Transformer

# ES
from ESRNN.ES import _ESM, _ES
...
################################################# _ESTransformer ################################################# 
class _ESTransformer(nn.Module):
  def __init__(self, mc):
    super(_ESTransformer, self).__init__()
    self.mc = mc
    self.es = _ESM(mc).to(self.mc.device)
    #self.transformer = _Transformer(mc).to(self.mc.device)
    self.transformer = _Transformer(mc).to(self.mc.device)
...
class ESTransformer(object):
  ...
  def train(self, dataloader, max_epochs, warm_start=False, shuffle=True, verbose=True):
    ...
    if self.mc.ensemble:
      self.estransformer_ensemble = [deepcopy(self.estransformer).to(self.mc.device)] * 5
    ...
    for epoch in range(max_epochs):
      ...
      for j in range(dataloader.n_batches):
        ...
        # Transformer doesn't need gradient clipping or normalization
        # torch.nn.utils.clip_grad_norm_(self.estransformer.transformer.parameters(), self.mc.gradient_clipping_threshold)
        # torch.nn.utils.clip_grad_norm_(self.estransformer.es.parameters(), self.mc.gradient_clipping_threshold)
  ...
  def instantiate_estransformer(self, exogenous_size, n_series):    
    ...
    self.estransformer = _ESTransformer(self.mc).to(self.mc.device)
    print(self.estransformer)
